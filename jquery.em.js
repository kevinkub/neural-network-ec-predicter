...
var tools = {
    ...
    calculateResults : function(matches, isTieAllowed) {
      $.each(matches, function(i, m) {
        if (null == m.Score1 && null == m.Score2) {
          if (isTieAllowed) {
            var result = buildRequest(m.Team1.Title, m.Team2.Title);
            m.Score1 = 0; m.Score2 = 0;
			if(result.team1 > result.team2 * 2) {
				m.Score1++;
			} else if (result.team1 * 2 < result.team2) {
				m.Score2++;
			}
			if(result.team1 > 0.2) {
			m.Score1++;
    ...
    getArticle : function(group) {
      ...
      $.each(group.Matches, function(i, m) {
        ...
        if (m.Input1) {
          ...
          m.Input1 = tools.getInput(m.Score1, m.IsLocked).appendTo(div);
        }
        div.append('<span> : </span>');
        if (m.Input2) {
          m.Input2.appendTo(div);
        } else {
          m.Input2 = tools.getInput(m.Score2, m.IsLocked).appendTo(div);
        }
        return;
      });
      return article;
    ...
    getThirds : function(groups) {
      var teams = [];
      $.each(groups, function(i, group) {
        var team = group.Teams[2];
        team.Group = group.Title[group.Title.length - 1]; // Gruppennamen müssen mit großem Buchstaben der Gruppe enden
        teams.push(team);
      });
      teams.sort(function(a, b) {
        ...
        if (0 == r) {
          ...
          if (0 == r) {
            ...
            if (0 == r) {
              console.log("Die Drittplatzierten der Gruppen konnten nicht eindeutig ermittelt werden.");
            }
          }
        }
        return r;
      });
      var key = teams[0].Group + teams[1].Group + teams[3].Group + teams[2].Group;
      key = key.split('').sort().join('');
      if ('ABCD' == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[0].Teams[2], groups[1].Teams[2]];
      } else if ('ABCE' == key) {
        teams = [groups[2].Teams[2], groups[0].Teams[2], groups[1].Teams[2], groups[4].Teams[2]];
      } else if ('ABCF' == key) {
        teams = [groups[2].Teams[2], groups[0].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('ABDE' == key) {
        teams = [groups[3].Teams[2], groups[0].Teams[2], groups[1].Teams[2], groups[4].Teams[2]];
      } else if ('ABDF' == key) {
        teams = [groups[3].Teams[2], groups[0].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('ABEF' == key) {
        teams = [groups[4].Teams[2], groups[0].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('ACDE' == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[0].Teams[2], groups[4].Teams[2]];
      } else if ('ACDF' == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[0].Teams[2], groups[5].Teams[2]];
      } else if ('ACEF' == key) {
        teams = [groups[2].Teams[2], groups[0].Teams[2], groups[5].Teams[2], groups[4].Teams[2]];
      } else if ('ADEF' == key) {
        teams = [groups[3].Teams[2], groups[0].Teams[2], groups[5].Teams[2], groups[4].Teams[2]];
      } else if ("BCDE" == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[1].Teams[2], groups[4].Teams[2]];
      } else if ('BCDF' == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('BCEF' == key) {
        teams = [groups[4].Teams[2], groups[2].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('BDEF' == key) {
        teams = [groups[4].Teams[2], groups[3].Teams[2], groups[1].Teams[2], groups[5].Teams[2]];
      } else if ('CDEF' == key) {
        teams = [groups[2].Teams[2], groups[3].Teams[2], groups[5].Teams[2], groups[4].Teams[2]];
      } else {
        $.error('Die Drittplatzierungen der Gruppen sind ungültig.');
      }
      return teams;
    },
  ...
  var methods = {
    ...
    loadData : function() {
      this.each(function() {
        ...
        main.data('data', data);
        main.em('updateGroups');
        $.each(data.Groups, function(i, group) {
          tools.calculateResults(group.Matches, true);
          return;
        });
        main.em('updateGroups');
        main.em('updateRoundOf16');
        tools.calculateResults(data.RoundOf16.Matches, false);
        main.em('updateQuarterFinals');
        tools.calculateResults(data.QuarterFinals.Matches, false);
        main.em('updateSemiFinals');
        tools.calculateResults(data.SemiFinals.Matches, false);
        main.em('updateFinal');
        tools.calculateResults(data.Final.Matches, false);
        main.em('writeData');
        return;
      });
      return this;
    ...
    updateQuarterFinals : function() {
      this.each(function() {
        ...
        if (data.RoundOf16.Matches[0].Score1 > data.RoundOf16.Matches[0].Score2) {
          data.QuarterFinals.Matches[0].Team1 = data.RoundOf16.Matches[0].Team1;
        } else {
          data.QuarterFinals.Matches[0].Team1 = data.RoundOf16.Matches[0].Team2;
        }
        if (data.RoundOf16.Matches[1].Score1 > data.RoundOf16.Matches[1].Score2) {
          data.QuarterFinals.Matches[0].Team2 = data.RoundOf16.Matches[1].Team1;
        } else {
          data.QuarterFinals.Matches[0].Team2 = data.RoundOf16.Matches[1].Team2;
        }
        if (data.RoundOf16.Matches[2].Score1 > data.RoundOf16.Matches[2].Score2) {
          data.QuarterFinals.Matches[1].Team1 = data.RoundOf16.Matches[2].Team1;
        } else {
          data.QuarterFinals.Matches[1].Team1 = data.RoundOf16.Matches[2].Team2;
        }
        if (data.RoundOf16.Matches[3].Score1 > data.RoundOf16.Matches[3].Score2) {
          data.QuarterFinals.Matches[1].Team2 = data.RoundOf16.Matches[3].Team1;
        } else {
          data.QuarterFinals.Matches[1].Team2 = data.RoundOf16.Matches[3].Team2;
        }
        if (data.RoundOf16.Matches[4].Score1 > data.RoundOf16.Matches[4].Score2) {
          data.QuarterFinals.Matches[2].Team1 = data.RoundOf16.Matches[4].Team1;
        } else {
          data.QuarterFinals.Matches[2].Team1 = data.RoundOf16.Matches[4].Team2;
        }
        if (data.RoundOf16.Matches[5].Score1 > data.RoundOf16.Matches[5].Score2) {
          data.QuarterFinals.Matches[2].Team2 = data.RoundOf16.Matches[5].Team1;
        } else {
          data.QuarterFinals.Matches[2].Team2 = data.RoundOf16.Matches[5].Team2;
        }
        if (data.RoundOf16.Matches[6].Score1 > data.RoundOf16.Matches[6].Score2) {
          data.QuarterFinals.Matches[3].Team1 = data.RoundOf16.Matches[6].Team1;
        } else {
          data.QuarterFinals.Matches[3].Team1 = data.RoundOf16.Matches[6].Team2;
        }
        if (data.RoundOf16.Matches[7].Score1 > data.RoundOf16.Matches[7].Score2) {
          data.QuarterFinals.Matches[3].Team2 = data.RoundOf16.Matches[7].Team1;
        } else {
          data.QuarterFinals.Matches[3].Team2 = data.RoundOf16.Matches[7].Team2;
        }
        return;
      });
      return this;
    ...
    updateRoundOf16 : function() {
      this.each(function() {
        var data = $(this).data('data');
        var thirds = tools.getThirds(data.Groups);
        data.RoundOf16.Matches[0].Team1 = data.Groups[0].Teams[1];
        data.RoundOf16.Matches[0].Team2 = data.Groups[2].Teams[1];
        data.RoundOf16.Matches[1].Team1 = data.Groups[3].Teams[0];
        data.RoundOf16.Matches[1].Team2 = thirds[3];
        data.RoundOf16.Matches[2].Team1 = data.Groups[1].Teams[0];
        data.RoundOf16.Matches[2].Team2 = thirds[1];
        data.RoundOf16.Matches[3].Team1 = data.Groups[5].Teams[0];
        data.RoundOf16.Matches[3].Team2 = data.Groups[4].Teams[1];
        data.RoundOf16.Matches[4].Team1 = data.Groups[2].Teams[0];
        data.RoundOf16.Matches[4].Team2 = thirds[2];
        data.RoundOf16.Matches[5].Team1 = data.Groups[4].Teams[0];
        data.RoundOf16.Matches[5].Team2 = data.Groups[3].Teams[1];
        data.RoundOf16.Matches[6].Team1 = data.Groups[0].Teams[0];
        data.RoundOf16.Matches[6].Team2 = thirds[0];
        data.RoundOf16.Matches[7].Team1 = data.Groups[1].Teams[1];
        data.RoundOf16.Matches[7].Team2 = data.Groups[5].Teams[1];
        return;
      });
      return this;
    },
    ...
    updateSemiFinals : function() {
      this.each(function() {
        ...
        if (data.QuarterFinals.Matches[2].Score1 > data.QuarterFinals.Matches[2].Score2) {
          data.SemiFinals.Matches[1].Team1 = data.QuarterFinals.Matches[2].Team1;
        } else {
          data.SemiFinals.Matches[1].Team1 = data.QuarterFinals.Matches[2].Team2;
        }
        if (data.QuarterFinals.Matches[3].Score1 > data.QuarterFinals.Matches[3].Score2) {
          data.SemiFinals.Matches[1].Team2 = data.QuarterFinals.Matches[3].Team1;
        } else {
          data.SemiFinals.Matches[1].Team2 = data.QuarterFinals.Matches[3].Team2;
        }
        return;
      });
      return this;
  ...
  $.fn.em = function(method) {
    if (methods[method]) {
      return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
    } else if (typeof method === 'object' || !method) {
      return methods.init.apply(this, arguments);
    } else {
      $.error('Method ' +  method + ' does not exist on jQuery.em.');
    }
  };
